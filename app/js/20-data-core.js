/* PWADC Security Operations Suite v4.4.0 | module: data-core */
...
function recordModuleLoadInfo(module,res){
  ...
  moduleLoadInfo[module]={module,source:res&&res.source||'unknown',sourceDetail:res&&res.sourceDetail||'',path:res&&res.path||'',fileModified:res&&res.fileModified||'',loadedAt:res&&res.loadedAt||new Date().toLocaleString(),dataRoot:res&&res.dataRoot||settings.dataRoot||'',liveFileExisted:!!(res&&res.liveFileExisted),revision:res&&res.revision||'missing',schemaVersion:res&&res.schemaVersion||'',expectedSchemaVersion:res&&res.expectedSchemaVersion||'',lastWrittenByAppVersion:res&&res.lastWrittenByAppVersion||'',schemaStatus:res&&res.schemaStatus||'unknown',schemaMessage:res&&res.schemaMessage||'',writeAllowed:res&&res.writeAllowed!==undefined?!!res.writeAllowed:true,conflict:false};
}
...
function markModuleRestored(module,path,revision='',res={}){moduleLoadInfo[module]={module,source:'restored-backup',sourceDetail:'Restored from previewed backup.',path:path||'',fileModified:new Date().toLocaleString(),loadedAt:new Date().toLocaleString(),dataRoot:settings.dataRoot||'',revision:revision||'',schemaVersion:res.schemaVersion||'',expectedSchemaVersion:res.expectedSchemaVersion||'',lastWrittenByAppVersion:res.lastWrittenByAppVersion||'',schemaStatus:res.schemaStatus||'current',schemaMessage:res.schemaMessage||'Schema is current.',writeAllowed:res.writeAllowed!==undefined?!!res.writeAllowed:true,conflict:false};}
function updateModuleRevisionAfterSave(module,res){moduleLoadInfo=moduleLoadInfo&&typeof moduleLoadInfo==='object'?moduleLoadInfo:{};const prior=moduleLoadInfo[module]||{};moduleLoadInfo[module]={...prior,module,source:'live-shared',sourceDetail:'Saved and revision-verified against the shared Data folder.',path:res&&res.path||prior.path||'',fileModified:res&&res.savedAt||new Date().toLocaleString(),loadedAt:new Date().toLocaleString(),dataRoot:settings.dataRoot||prior.dataRoot||'',liveFileExisted:true,revision:res&&res.revision||res&&res.sha256||prior.revision||'',schemaVersion:res&&res.schemaVersion||prior.schemaVersion||'',expectedSchemaVersion:res&&res.expectedSchemaVersion||prior.expectedSchemaVersion||'',lastWrittenByAppVersion:res&&res.lastWrittenByAppVersion||prior.lastWrittenByAppVersion||'',schemaStatus:res&&res.schemaStatus||'current',schemaMessage:res&&res.schemaMessage||'Schema is current.',writeAllowed:res&&res.writeAllowed!==undefined?!!res.writeAllowed:true,conflict:false};}
function migrationStatusItems(){return [...(schemaMigrationStatus.pending||[]),...(schemaMigrationStatus.blocked||[])]}
function migrationStatusBanner(){const pending=schemaMigrationStatus.pending||[],blocked=schemaMigrationStatus.blocked||[];if(!pending.length&&!blocked.length)return'';const total=pending.length+blocked.length;const msg=pending.length?`${pending.length} module(s) require authorized migration approval.`:`${blocked.length} module(s) have schema migration blocks that require review.`;const action=hasCapability('schema.manage')&&pending.length?`<button class="sm" onclick="showNextSchemaMigrationModal()">Review Migration</button>`:'';return `<div class="notice warn"><strong>Protected Data Upgrade:</strong> ${esc(msg)} Affected modules remain read-only until resolved. ${action}<div class="mini-note">${esc(migrationStatusItems().map(x=>`${x.moduleLabel||moduleLabel(x.module)}: ${x.sourceSchema||'unknown'} → ${x.targetSchema||'unknown'}`).join(' · '))}</div></div>`}
async function refreshSchemaMigrationStatus(){try{schemaMigrationStatus=await SuiteBridge.send('suite:getMigrationStatus')||{pending:[],blocked:[],completedAutomatically:[],warnings:[]};ensureAllowedModule();return schemaMigrationStatus}catch(e){toast('Migration status refresh failed: '+e.message);return schemaMigrationStatus}}
function handlePendingSchemaMigrationsAfterLogin(){const pending=schemaMigrationStatus.pending||[],blocked=schemaMigrationStatus.blocked||[];ensureAllowedModule();if(hasCapability('schema.manage')&&pending.length){showNextSchemaMigrationModal();return;}if(!hasCapability('schema.manage')&&pending.length){toast('An authorized data upgrade is pending. Affected modules are unavailable until it is approved.');return;}if(blocked.length){toast('One or more data modules are read-only because schema review is required.');}}
function schemaMigrationCountsHtml(item){const rows=Object.entries(item&&item.recordCounts||{});if(!rows.length)return'<div class="mini-note">No module record counters were available.</div>';return `<div class="settings-table-wrap"><table><thead><tr><th>Record Group</th><th>Count</th></tr></thead><tbody>${rows.map(([k,v])=>`<tr><td>${esc(k)}</td><td>${esc(v)}</td></tr>`).join('')}</tbody></table></div>`}
function showNextSchemaMigrationModal(){if(!hasCapability('schema.manage')){toast('Schema management permission is required for migration approval.');return;}const item=(schemaMigrationStatus.pending||[])[0];if(!item){toast('No migration approval is pending.');return;}showModal(`<div class="modal-head"><div><div class="modal-title">Data Migration Review · ${esc(item.moduleLabel||moduleLabel(item.module))}</div><div class="mini-note">${esc(item.sourceSchema||'')} → ${esc(item.targetSchema||'')}</div></div><button onclick="closeModal()">Close</button></div><div class="notice ${item.businessMeaningChanged?'warn':''}"><strong>${item.businessMeaningChanged?'Business-rule migration':'Structural data migration'}</strong><br>${esc(item.summary||'Approved schema migration is required before this module can accept writes.')}</div><div class="card"><div class="card-title">What Will Change</div>${(item.changes||[]).length?`<ul class="clean-list">${item.changes.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:'<p class="mini-note">No additional change notes were supplied.</p>'}</div><div class="card"><div class="card-title">Pre-Migration Backup</div><div><strong>${esc(item.backupCreatedAt||'Created during startup review')}</strong></div><div class="mini-note">${esc(item.backupPath||'Backup path unavailable')}</div></div><div class="card"><div class="card-title">Records Checked Before Migration</div>${schemaMigrationCountsHtml(item)}</div><div class="notice">Migration is all-or-nothing for this module. The converted data is validated in memory, written through the protected persistence path, reopened from disk, and verified before the module is unlocked. If post-write verification fails, the pre-migration file is restored.</div><div class="modal-actions"><button onclick="closeModal()">Cancel / Keep Read-Only</button><button class="primary" onclick="approvePendingSchemaMigration('${esc(item.module)}')">Approve Migration</button></div>`)}
async function approvePendingSchemaMigration(module){if(!hasCapability('schema.manage')||!currentUser){toast('Schema management permission is required.');return;}const btn=document.querySelector('#modalBackdrop .modal-actions .primary');if(btn){btn.disabled=true;btn.textContent='Migrating...';}try{const res=await SuiteBridge.send('suite:approveSchemaMigration',{module,adminUserId:currentUser.id,adminPin:currentUser.pin||''});schemaMigrationStatus=res&&res.status||schemaMigrationStatus;await reloadDataModuleAfterMigration(module);closeModal();ensureAllowedModule();renderShell();safeRenderPages();toast(`${moduleLabel(module)} migration completed and verified`);if((schemaMigrationStatus.pending||[]).length)setTimeout(showNextSchemaMigrationModal,120);}catch(e){if(btn){btn.disabled=false;btn.textContent='Approve Migration';}toast('Migration failed: '+e.message)}}
async function reloadDataModuleAfterMigration(module){if(module==='attendance')return loadAttendance();if(module==='roster')return loadRoster();if(module==='tasks')return loadTasks();if(module==='shift-reports')return loadShiftReports();if(module==='shift-intelligence')return loadShiftIntel();if(module==='suite-settings'){const res=await SuiteBridge.send('suite:getSettings');settings=normalizeSettings(res.settings||{});env=res.environment||env;applyTheme();updateUserStatus();return;}return null}
...
function employeeProfileIdForRoster(e){return e&&e.id!==undefined?String(e.id):''}
function employeeProfileIdForAttendance(ae){if(!ae)return '';const re=rosterEmployeeForAttendance(ae);return re&&re.id!==undefined?String(re.id):String(ae.id||'')}
function rosterEmployeeForDisplayName(name){const raw=String(name||'').trim();if(!raw)return null;return (roster.employees||[]).find(e=>scheduleNameMatchesEmployee(raw,e)||normalizeNameKey(fullName(e))===normalizeNameKey(raw)||normalizeNameKey(rosterEmployeeAttendanceName(e))===normalizeNameKey(raw))||null}
function employeeProfileLink(id,label,className=''){const key=String(id||'');const text=String(label||'').trim();if(!key||!text)return esc(text);return `<button type="button" class="employee-profile-link ${esc(className)}" onclick="event.stopPropagation();openEmployeeProfile('${esc(key)}')" title="Open ${esc(text)} profile">${esc(text)}</button>`}
function rosterEmployeeProfileLink(e,label=''){return employeeProfileLink(employeeProfileIdForRoster(e),label||fullName(e))}
function attendanceEmployeeProfileLink(e,label=''){return employeeProfileLink(employeeProfileIdForAttendance(e),label||(e&&e.name)||'')}
function employeeProfileLinkByName(name){const re=rosterEmployeeForDisplayName(name);if(re)return rosterEmployeeProfileLink(re,name);const ae=(attendance.employees||[]).find(a=>normalizeNameKey(a.name)===normalizeNameKey(name));return ae?attendanceEmployeeProfileLink(ae,name):esc(name||'')}
...
function profileRecentAttendance(ae,limit=10){if(!ae)return[];const data=(attendance.attendance||{})[ae.id]||{};return Object.entries(data).filter(x=>x[1]).sort((a,b)=>b[0].localeCompare(a[0])).slice(0,limit).map(([date,code])=>({date,code,label:pointCodeLabel(code)}));}
...
async function saveModuleDataStrict(module,obj){
  ...
  if(info.writeAllowed===false){throw new Error(`${module} is read-only because its data schema is not compatible with this app. ${info.schemaMessage||''}`.trim());}
  ...
  try{
    const res=await SuiteBridge.send('suite:saveModuleData2',{module,json,expectedRevision},{authorization:authorizationEnvelope()});

/* PWADC Security Operations Suite v4.2.0 | module: data-core */
...
function handlePendingSchemaMigrationsAfterLogin(){const pending=schemaMigrationStatus.pending||[],blocked=schemaMigrationStatus.blocked||[];ensureAllowedModule();if(roleOf()==='Admin'&&pending.length){showNextSchemaMigrationModal();return;}if(roleOf()!=='Admin'&&pending.length){toast('An Administrator data upgrade is pending. Affected modules are unavailable until it is approved.');return;}if(blocked.length){toast('One or more data modules are read-only because schema review is required.');}}

/* PWADC Security Operations Suite v3.5.0.11 | module: shell-audits */
...
function moduleFileStatusHtml(){if(!moduleFileStatusRows)return '<div class="notice">Click <strong>Load File Status</strong> to inspect live JSON file paths, sizes, last saves, and newest backups.</div>';return `<div class="settings-table-wrap"><table><thead><tr><th>Module</th><th>Live File</th><th>Status</th><th>Size</th><th>Modified</th><th>Newest Data Date</th><th>lastSaved</th><th>Source</th><th>Newest Backup</th></tr></thead><tbody>${moduleFileStatusRows.map(r=>`<tr><td>${esc(r.label||r.module||'')}</td><td><div>${esc(r.fileName||'')}</div><div class="mini-note">${esc(r.path||'')}</div></td><td class="${r.error||r.integrityStatus==='invalid'?'bad':r.exists?'ok':'warn'}">${r.error?esc(r.error):(r.integrityStatus==='invalid'?'INVALID JSON':(r.exists?'VALID':'MISSING'))}${r.integrityError?`<div class="mini-note">${esc(r.integrityError)}</div>`:''}</td><td>${r.sizeBytes?Math.round(Number(r.sizeBytes)/1024)+' KB':''}</td><td>${esc(r.modified||'')}</td><td class="${r.newestDataDate?freshnessClass(r.newestDataDate,3,14):'warn'}">${esc(fmt(r.newestDataDate||''))}</td><td>${esc(r.lastSaved||'')}</td><td>${sourceBadge({source:r.sourceStatus||'unknown'})}</td><td><div>${esc(r.newestBackup||'')}</div><div class="mini-note">${esc(r.newestBackupModified||'')}</div></td></tr>`).join('')}</tbody></table></div>`;}

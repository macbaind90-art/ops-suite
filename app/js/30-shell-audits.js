/* PWADC Security Operations Suite v4.2.0 | module: shell-audits */
...
function navigate(id){if(!canAccessModule(id)){toast(id==='data-health'?'Data Health & Recovery is available to Administrators only.':'Access denied for '+id);id='home';}activeModule=id;updateNavActive();document.getElementById('moduleStatus').textContent=MODULES.find(m=>m.id===id)?.label||id;updateUserStatus();safeRenderPages();if(id==='data-health'&&canAdmin()&&typeof openDataHealthDashboard==='function')openDataHealthDashboard()}

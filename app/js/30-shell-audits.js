/* PWADC Security Operations Suite v4.4.0 | module: shell-audits */
...
function navHealthClass(m){return m.id==='data-health'&&typeof healthIndicator!=='undefined'?` health-nav-${healthIndicator.severity||'gray'}`:''}
function renderNavGroup(g,visible){const items=(g.items||[]).map(id=>visible.find(m=>m.id===id)).filter(Boolean);if(!items.length)return '';if(items.length===1){const m=items[0];return `<button class="nav-single${navHealthClass(m)}" data-module="${m.id}" onclick="navigate('${m.id}')">${esc(navDisplayLabel(m))}</button>`;}return `<div class="nav-group" data-group="${esc(g.label)}"><button class="nav-group-btn" type="button">${esc(g.label)} ▾</button><div class="nav-menu">${items.map(m=>`<button class="${navHealthClass(m).trim()}" data-module="${m.id}" onclick="navigate('${m.id}')">${esc(navDisplayLabel(m))}</button>`).join('')}</div></div>`}
...
function computeDataHealth(){const issues=[];const add=(severity,module,title,detail)=>issues.push({severity,module,title,detail});const ri=buildRosterIndexes(),ai=buildAttendanceIndexes();
 ...
 for(const a of Array.isArray(attendance.correctiveActions)?attendance.correctiveActions:[]){if(!String(a.empId||'').trim())add('warning','Attendance Points','Corrective action missing employee link','A corrective-action record has no empId.');if(!String(a.level||'').trim()||!String(a.date||'').trim())add('warning','Attendance Points','Incomplete corrective action',(a.employee||'Corrective action')+' is missing level or action date.');}for(const a of Array.isArray(attendance.pointAdjustments)?attendance.pointAdjustments:[]){if(!String(a.empId||'').trim())add('warning','Attendance Points','Point adjustment missing employee link','A point-adjustment record has no empId.');if(!String(a.reason||'').trim())add('warning','Attendance Points','Point adjustment missing reason',(a.employee||'Point adjustment')+' does not document a reason.');}if(attendance.pointSystem&&attendance.pointSystem.migrationPending)add('warning','Attendance Points','Point migration pending','Attendance is loaded but the current point-system migration has not been committed.');
...
async function alignAttendanceNamesToRoster(){
  if(!hasCapability('attendance.edit')){toast('Attendance edit permission is required');return;}
...
async function repairMissingEmployeeIds(){
  if(!hasCapability('attendance.edit')||!hasCapability('roster.edit')||!hasCapability('schedule.edit')){toast('Attendance, roster, and schedule edit permissions are required');return;}
...
async function removeBlankScheduleRows(){
  if(!hasCapability('schedule.edit')){toast('Schedule edit permission is required');return;}
...
function dataHealthFixPanel(){return `<div class="card"><div class="card-title">Fix Actions</div><div class="notice warn">Fix buttons create backups before changing live data. They preserve attendance history unless the action clearly says otherwise.</div><div class="toolbar" style="margin-top:10px"><button class="gold" data-capability="attendance.edit" onclick="alignAttendanceNamesToRoster()">Sync Roster to Attendance</button><button class="gold" data-capability="attendance.edit" onclick="archiveAttendanceNotInRoster()">Archive Attendance Not on Roster</button><button data-capability="roster.edit" onclick="repairMissingEmployeeIds()">Repair Missing IDs</button><button data-capability="schedule.edit" onclick="removeBlankScheduleRows()">Remove Blank Schedule Rows</button></div></div>`;}
...
async function runBackupCleanup(){if(!canRestoreData()){toast('Backup cleanup requires the data.restore capability');return;}if(!backupCleanupPreview||backupCleanupPreview.loading||backupCleanupPreview.error){toast('Run cleanup preview first');return;}const count=Number(backupCleanupPreview.deleteCount||0);if(!count){toast('No cleanup candidates found');return;}if(!confirm(`Delete ${count} cleanup candidate backup file(s)? Manual, archive, legacy, and protected backups are kept.`))return;if(!confirm('Final confirmation: this deletes only the previewed cleanup candidates and writes a cleanup log. Continue?'))return;try{const r=await SuiteBridge.send('suite:cleanupBackups',{module:backupCleanupModule||'all'},{authorization:authorizationEnvelope()});toast(`Backup cleanup deleted ${r.deletedCount||0} file(s); recovered ${bytesLabel(r.bytesRecovered||0)}.`);backupCleanupPreview=null;await loadBackupManager();}catch(e){toast('Backup cleanup failed: '+e.message)}}
...
function moduleFileStatusHtml(){if(!moduleFileStatusRows)return '<div class="notice">Click <strong>Load File Status</strong> to inspect live JSON paths, schema versions, sizes, last saves, and newest backups.</div>';return `<div class="settings-table-wrap"><table><thead><tr><th>Module</th><th>Live File</th><th>Status</th><th>Schema</th><th>Size</th><th>Modified</th><th>Newest Data Date</th><th>lastSaved</th><th>Source</th><th>Newest Backup</th></tr></thead><tbody>${moduleFileStatusRows.map(r=>{const schemaOk=r.schemaStatus==='current';return `<tr><td>${esc(r.label||r.module||'')}</td><td><div>${esc(r.fileName||'')}</div><div class="mini-note">${esc(r.path||'')}</div></td><td class="${r.error||r.integrityStatus==='invalid'?'bad':r.exists?'ok':'warn'}">${r.error?esc(r.error):(r.integrityStatus==='invalid'?'INVALID JSON':(r.exists?'VALID':'MISSING'))}${r.integrityError?`<div class="mini-note">${esc(r.integrityError)}</div>`:''}</td><td class="${schemaOk?'ok':r.exists?'warn':''}">${esc(r.schemaVersion||r.expectedSchemaVersion||'')}<div class="mini-note">${esc(r.schemaStatus||'')}${r.lastWrittenByAppVersion?' · app '+esc(r.lastWrittenByAppVersion):''}</div>${r.schemaMessage&&!schemaOk?`<div class="mini-note">${esc(r.schemaMessage)}</div>`:''}</td><td>${r.sizeBytes?Math.round(Number(r.sizeBytes)/1024)+' KB':''}</td><td>${esc(r.modified||'')}</td><td class="${r.newestDataDate?freshnessClass(r.newestDataDate,3,14):'warn'}">${esc(fmt(r.newestDataDate||''))}</td><td>${esc(r.lastSaved||'')}</td><td>${sourceBadge({source:r.sourceStatus||'unknown'})}</td><td><div>${esc(r.newestBackup||'')}</div><div class="mini-note">${esc(r.newestBackupModified||'')}</div></td></tr>`}).join('')}</tbody></table></div>`;}

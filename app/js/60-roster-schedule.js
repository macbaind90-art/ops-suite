/* PWADC Security Operations Suite v4.3.0 | module: roster-schedule */
...
function openManageRanksModal(){if(!hasCapability('roster.edit')){toast('Manage ranks is Admin-only');return;}
  roster.customRanks=Array.isArray(roster.customRanks)?roster.customRanks:[];
...
async function saveEmployeeModal(id=''){if(!hasCapability('roster.edit')){toast('Roster editing is Admin-only');return;}
  try{
    ...
    let attendanceSyncResult=null;
    try{
      attendanceSyncResult=await syncOneRosterEmployeeToAttendance(e,true);
      const attendanceOk=await saveAttendanceNow(wasNewEmployee?'roster-add-employee-sync':'roster-edit-employee-sync');
      const attendanceMatch=findAttendanceEmployeeForRoster(e);
      if(!attendanceOk||!attendanceMatch||isArchivedAttendanceEmployee(attendanceMatch)){
        closeModal();safeRenderPages();
        toast('Roster saved, but Attendance sync did not persist. Resolve the Attendance save/conflict warning, then use Sync Roster to Attendance.');
        return;
      }
    }catch(syncErr){
      reportActionError('Roster to Attendance sync',syncErr);
      closeModal();safeRenderPages();
      toast('Roster saved, but Attendance sync failed. Use Sync Roster to Attendance after resolving the error.');
      return;
    }
    closeModal();safeRenderPages();toast(wasNewEmployee?'Roster employee added and verified in Attendance':'Roster employee saved and verified in Attendance');
...
async function removeRosterEmployee(id){if(!hasCapability('roster.edit')){toast('Roster removal is Admin-only');return;}
...
function confirmMerit(id){if(!hasCapability('roster.edit')){toast('Merit changes are Admin-only');return;}let e=roster.employees.find(x=>String(x.id)===String(id));if(!e)return;let base=Number(e.rate||0),amt=Number(val('meritAmt')||0),type=val('meritType');let next=type==='percent'?base+(base*amt/100):base+amt;e.rate=Number(next.toFixed(2));e.dop=val('meritDate')||e.dop;roster.audit.push({at:new Date().toISOString(),user:currentUserName()||env.user||'',action:'Merit increase',detail:fullName(e)+' '+money(base)+' -> '+money(e.rate)});saveRosterNow('merit').then(ok=>{if(ok){closeModal();safeRenderPages();toast('Merit increase applied')}})}
...
function renderScheduleWorkspaceBanner(){const d=activeScheduleDraft();const drafts=scheduleDraftList().length;const clip=scheduleClipboard?`<span class="schedule-clipboard">Copied: ${esc(scheduleClipboard.value||'Closed')} · ${esc(scheduleClipboard.source||'schedule cell')}</span>`:'';if(d)return `<div class="schedule-workspace-banner mock"><div><span class="schedule-mode-pill mock">Mock / Not Published</span><strong>${esc(d.name||'Untitled Mock Schedule')}</strong><span>${esc(d.notes||'Draft staffing scenario. Changes here do not affect live HPW, labor analytics, attendance, or management reports.')}</span></div><div class="schedule-workspace-tools">${clip}<button onclick="openMockScheduleManager()">Mock Schedules (${drafts})</button><button onclick="returnToLiveSchedule()">Return to Live</button><button class="gold" data-capability="schedule.publish" onclick="openApplyMockScheduleModal('${esc(d.id)}')">Apply to Live</button></div></div>`;return `<div class="schedule-workspace-banner"><div><span class="schedule-mode-pill">Live / Published</span><strong>Live Master Schedule</strong><span>This is the schedule authority used for HPW, staffing reports, and labor analytics. ${drafts} mock schedule${drafts===1?' is':'s are'} being held separately.</span></div><div class="schedule-workspace-tools">${clip}<button onclick="openMockScheduleManager()">Mock Schedules (${drafts})</button><button class="gold" onclick="openCreateMockScheduleModal('live')">Save Current as Mock</button></div></div>`}
...
function duplicateScheduleRow(idx){if(!hasCapability('schedule.edit')){toast('Schedule editing is Admin-only');return;}const rows=scheduleWorkspaceRows();const row=rows[idx];if(!row)return;const copy=JSON.parse(JSON.stringify(row));copy.post=(copy.post||'Post')+' Copy';copy.autoAdded=false;rows.splice(idx+1,0,copy);touchScheduleDraft();scheduleColorCache=null;scheduleWorkspaceAudit(scheduleWorkspaceIsDraft()?'Mock schedule row duplicated':'Schedule row duplicated',scheduleWorkspaceDetail((row.section||'')+' · '+(row.post||'')));saveRoster('schedule-row-duplicate');safeRenderPages();toast(scheduleWorkspaceIsDraft()?'Mock schedule row duplicated':'Schedule row duplicated');}
function openAddScheduleRowModal(){if(!hasCapability('schedule.edit')){toast('Schedule editing is Admin-only');return;}
...
function openAddScheduleSectionModal(){if(!hasCapability('schedule.edit')){toast('Schedule editing is Admin-only');return;}
...
function openEditScheduleRowModal(idx){if(!hasCapability('schedule.edit')){toast('Schedule editing is Admin-only');return;}
...
function openScheduleCellModal(idx,day){if(!hasCapability('schedule.edit')){toast('Schedule editing is Admin-only');return;}

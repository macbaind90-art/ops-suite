/* PWADC Security Operations Suite v4.2.1 | module: bootstrap */
...
function canAccessModule(id){
  ...
  if(role!=='Admin'&&migrationModuleNeedsAdmin(migrationModule))return false;
...
function unlockSuite(){unlocked=true;document.getElementById('lockScreen').classList.add('hidden');document.getElementById('app').classList.remove('hidden');ensureAllowedModule();updateUserStatus();renderShell();const startHealth=canAdmin()&&env.openDataHealthAfterLogin;const target=startHealth?'data-health':(canAccessModule(settings.defaultModule)?settings.defaultModule:'home');navigate(target);setTimeout(()=>{if(typeof handlePendingSchemaMigrationsAfterLogin==='function')handlePendingSchemaMigrationsAfterLogin();if(canAdmin()&&typeof loadHealthIndicator==='function')loadHealthIndicator()},80)}

/* PWADC Security Operations Suite v3.4.0.0 | module: workflows-home */
...
function renderStartHere(){
  ...
  return `<div class="page-head"><div><div class="page-title">Start Here</div><div class="page-sub">v3.4.0.0 Atomic Save + Integrity Foundation · validated JSON transactions and live-file protection.</div></div><div class="btn-group"><button onclick="navigate('home')">Dashboard</button><button class="gold" onclick="printStartHere()">Print Start Here</button></div></div>${screenGuide('start-here')}<div class="notice"><strong>Release posture:</strong> v3.4.0.0 hardens the shared JSON data layer. Critical saves now validate, preserve the current live file, durably stage a temporary file, replace the live file only after verification, and record a write audit.</div><div class="card"><div class="card-title">Daily Operating Flow</div><ol class="clean-list"><li><strong>Start on the Command Center.</strong> Work the priority queues that require action today.</li><li><strong>Maintain people and coverage records.</strong> Attendance, roster, schedule, training, and uniforms remain the operational source records.</li><li><strong>Process Shift Operations.</strong> Import reports, decide operational meaning, and move owned follow-up into Task Tracker.</li><li><strong>Use Reports for management closure.</strong> Select the management question, confirm scope, preview, then publish.</li><li><strong>Verify before high-impact action.</strong> Data Health and Backup & Restore are the control gate before repairs, cleanup, or restore.</li><li><strong>Use Change Log and Settings for governance.</strong> Review accountability, roles, assumptions, and restricted controls.</li></ol></div><div class="grid cols-3"><div class="kpi"><div class="num">10</div><div class="lbl">Front-End Functional Modules</div></div><div class="kpi"><div class="num">7</div><div class="lbl">Windows Host Source Files</div></div><div class="kpi"><div class="num">Guarded</div><div class="lbl">Ordered Startup</div></div></div><div class="card"><div class="card-title">Governance Lane</div><div class="governance-path"><div class="governance-step"><strong>1. Report</strong><span>Choose the output that answers the management decision.</span></div><div class="governance-step"><strong>2. Verify</strong><span>Review data integrity, live files, and backup posture.</span></div><div class="governance-step"><strong>3. Recover</strong><span>Preview scope and backup before replacing live data.</span></div><div class="governance-step"><strong>4. Govern</strong><span>Review audit activity and save controlled settings.</span></div></div></div><div class="card"><div class="card-title">What Is Included</div><div class="grid cols-3">${included.map(x=>moduleIncludedCard(x[0],x[1],x[2])).join('')}</div></div><div class="card"><div class="card-title">v3.4.0.0 Release Notes</div><table><thead><tr><th>Area</th><th>Controlled Improvement</th></tr></thead><tbody><tr><td>Atomic Persistence</td><td>Critical JSON saves stage and flush a temporary file, validate it, then replace the live file through a same-directory transaction.</td></tr><tr><td>Pre-Write Recovery</td><td>Existing live JSON is preserved to the module backup folder before replacement.</td></tr><tr><td>Integrity Guard</td><td>Malformed live JSON blocks normal saves instead of being silently overwritten.</td></tr><tr><td>Verification</td><td>SHA-256 and JSON parsing verify the temporary write and final live file before the transaction is reported successful.</td></tr><tr><td>Audit</td><td>Write results record user, workstation, module, operation, method, hash, backup path, and success state under Data Integrity.</td></tr></tbody></table></div>`
}

function printStartHere(){openReportWindow(reportDoc('Start Here','v3.4.0.0 Atomic Save + Integrity Foundation',document.querySelector('#page-start-here')?.innerHTML||renderStartHere(),'portrait'),false)}
...
function renderHome(){
 ...
 return `<div class="page-head"><div><div class="page-title">Security Operations Command Center</div><div class="page-sub">${env.user||'User'} @ ${env.machine||'Machine'} · v3.4.0.0</div></div><div class="btn-group"><button onclick="navigate('start-here')">Start Here</button><button onclick="navigate('shift-reports')">Import Reports</button><button class="gold" onclick="navigate('shift-intelligence')">Review Intelligence</button><button class="admin-only" onclick="backupEverything()">Backup</button></div></div>

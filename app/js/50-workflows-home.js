/* PWADC Security Operations Suite v3.5.0.8 | module: workflows-home */
  ...
  try{
    ...
    for(const emp of emps){
      ...
      if(sched.scheduled){
        ...
        else if(isChargeableAttendanceCode(code))out.issuesToday++;
      }
    }
  }catch(_e){}
  return out;
...
function renderHome(){
 ...
 return `<div class="page-head"><div><div class="page-title">Security Operations Command Center</div><div class="page-sub">${env.user||'User'} @ ${env.machine||'Machine'} · v3.5.0.8</div></div><div class="btn-group"><button onclick="navigate('start-here')">Start Here</button><button onclick="activeAttView='daily';navigate('attendance')">Daily Attendance</button><button class="gold" onclick="activeAttView='review';navigate('attendance')">Point Review</button><button class="admin-only" onclick="backupEverything()">Backup</button></div></div>

/* PWADC Security Operations Suite v4.7.0 | module: workflows-home */
...
function homeAttendanceMetrics(asOf=attendanceLocalToday()){
  ...
  try{
    ...
    for(const emp of emps){
      ...
      if(pts>=6)out.atThreshold++;
      if(pts>=9)out.highRisk++;
      ...
      if(correctiveActionWorkflow(emp.id,snap).due)out.correctiveDue++;
...
function peopleWorkflowNav(active=''){
  ...
  const buttons=[
    ...
    {id:'points',label:'Point Review',count:m.attendanceThreshold,sub:'6+ points',cmd:"activeAttView='review';navigate('attendance')"},
    {id:'actions',label:'Notice Control',count:m.attendanceDue,sub:'notice due',cmd:"activeAttView='actions';navigate('attendance')"},
  ...
  return `<div class="people-flow-strip"><div class="people-flow-head"><div><div class="people-flow-title">People Workflow</div><div class="people-flow-note">Use one current workflow: complete Daily Attendance, review active points and positive credits, manage attendance notices when due, then maintain roster, training, and uniforms.</div></div><div class="people-flow-kpis">${peopleFlowChip('Active Roster',m.activeRoster)}${peopleFlowChip('Attendance Employees',m.attendanceActive)}${peopleFlowChip('Open People Items',issueTotal,issueTotal?'warn':'')}</div></div><div class="people-flow-actions">${buttons.map(b=>`<button class="${active===b.id?'active':''}" onclick="${b.cmd}">${esc(b.label)}<span>${esc(b.count)} ${esc(b.sub)}</span></button>`).join('')}</div></div>`;
}
...
function renderEmployeePeopleCommand(re,ae){
  ...
  const flow=ae&&snap?correctiveActionWorkflow(ae.id,snap):null;
  const actionDue=!!(flow&&flow.due);
  ...
  const pointKind=pts>=9?'bad':pts>=6?'warn':'';
  const total=(actionDue?1:0)+trainingCount+uniformCount;
  const actionLabel=!snap?'No Attendance Record':actionDue?'Notice Due':flow&&flow.record?flow.status:snap.level==='None'?'Below Threshold':'Current';
  return `<div class="card people-command-panel"><div class="card-title">Employee People Command</div><div class="profile-priority-row"><div class="profile-priority-tile ${pointKind}"><b>${snap?esc(snap.activePoints):'-'}</b><span>Active Attendance Points</span></div><div class="profile-priority-tile"><b>${snap?esc(snap.bank)+' / '+esc(snap.maxCredits):'-'}</b><span>Positive Credit Bank</span></div><div class="profile-priority-tile"><b>${snap?esc(snap.cleanWorkingDays)+' / 12':'-'}</b><span>Clean Working Days</span></div><div class="profile-priority-tile ${actionDue?'bad':''}"><b>${esc(actionLabel)}</b><span>Attendance Notice</span></div><div class="profile-priority-tile ${trainingCount?'bad':''}"><b>${trainingCount}</b><span>Training Exceptions</span></div><div class="profile-priority-tile ${uniformCount?'warn':''}"><b>${uniformCount}</b><span>Uniform Follow-Ups</span></div></div><div class="people-page-note">${total?`This employee has ${total} people-work item(s) requiring review. Attendance notice decisions use current active points after positive credits.`:`No current notice, training, or uniform exception is flagged. ${schedCount} live schedule assignment(s) matched this employee.`}</div><div class="people-next-actions"><button onclick="activeAttView='review';navigate('attendance')">Point Review</button><button onclick="activeAttView='actions';navigate('attendance')">Notice Control</button><button onclick="activeAttView='grid';navigate('attendance')">90-Day Grid</button><button onclick="navigate('training')">Training</button><button onclick="activeRosterView='uniforms';navigate('roster')">Uniforms</button>${re?`<button class="gold" data-capability="roster.edit" onclick="openEmployeeModal('${esc(re.id)}')">Edit Roster Record</button>`:''}</div></div>`;
}
...
function renderHome(){
 ...
 const priorityItems=[
   ...
   {title:'Attendance Notice Due',value:att.correctiveDue,desc:'Employees who need a Notice at 6–8.99 points or a Final Warning at 9+ points.',btn:'Open Notice Control',go:"activeAttView='actions';navigate('attendance')",hot:att.correctiveDue>0,weight:95},
   {title:'Final Warning Threshold',value:att.highRisk,desc:'Employees at 9+ active points. Review the current balance, positive credits, and notice lifecycle.',btn:'Open Point Review',go:"activeAttView='review';navigate('attendance')",hot:att.highRisk>0,weight:90},
 ...
 const workflow=[
   ...
   ['Review Points & Notice Control','Review active 90-day points, positive credits, and any Notice or Final Warning due.','Points',"activeAttView='review';navigate('attendance')"],
 ...
 const secondary=[
   dashboardAttentionCard('Employees at 6+ Points',att.atThreshold,`${att.highRisk} employee(s) are at the 9+ Final Warning threshold. Positive credits are already reflected in active balances.`,'Point Review',"activeAttView='review';navigate('attendance')",att.atThreshold>0),
 ...
 <section class="command-layout"><div class="command-main"><div class="command-panel"><div class="command-panel-head"><div><div class="command-panel-title">Today’s Priority</div><div class="command-panel-sub">Lead with the highest-risk queue, then work the remaining signals.</div></div><button onclick="safeRenderPages()">Refresh</button></div>${commandPriorityCard(lead,true)}<div class="priority-compact-grid">${remaining.map(x=>commandPriorityCard(x,false)).join('')}</div></div><div class="command-panel"><div class="command-panel-head"><div><div class="command-panel-title">Daily Workflow</div><div class="command-panel-sub">A controlled operating sequence from attendance through closure.</div></div></div><div class="daily-flow">${workflow.map((x,i)=>commandWorkflowStep(i+1,x[0],x[1],x[2],x[3])).join('')}</div></div></div><aside class="command-sidebar"><div class="command-panel"><div class="command-panel-head"><div><div class="command-panel-title">Quick Actions</div><div class="command-panel-sub">Current attendance and coverage controls without legacy queues.</div></div></div><div class="sidebar-action-grid">${[
   ...
   ['Notice Control','Generate, issue, acknowledge, or record due attendance notices.','Open',"activeAttView='actions';navigate('attendance')"],

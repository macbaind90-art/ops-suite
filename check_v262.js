...
'use strict';
const MODULES=[{id:'home',label:'Home'},{id:'attendance',label:'Attendance'},{id:'roster',label:'Roster'},{id:'tasks',label:'Task Tracker'},{id:'other-programs',label:'Other Programs'}];
...
function renderModule(id){if(id==='home')return renderHome();if(id==='attendance')return renderAttendance();if(id==='roster')return renderRoster();if(id==='tasks')return renderTasks();if(id==='other-programs')return renderOtherPrograms();return `<div class="page-head"><div><div class="page-title">${MODULES.find(m=>m.id===id)?.label||id}</div><div class="page-sub">Module not available.</div></div></div>`}
...
function renderHome(){const flags=computeFlags();return `<div class="page-head"><div><div class="page-title">Security Operations Dashboard</div><div class="page-sub">${env.user||'User'} @ ${env.machine||'Machine'} · v2.6.2</div></div></div><div class="grid cols-3"><div class="kpi"><div class="num">${activeAttendanceEmployees().length}</div><div class="lbl">Active Attendance Employees</div></div><div class="kpi"><div class="num">${roster.employees.length}</div><div class="lbl">Roster Employees</div></div><div class="kpi"><div class="num">${flags.filter(f=>!f.addressed).length}</div><div class="lbl">Open Attendance Exceptions</div></div><div class="kpi"><div class="num">${(tasks.tasks||[]).filter(t=>t.status!=='Complete'&&t.status!=='On Hold').length}</div><div class="lbl">Active Tasks</div></div><div class="kpi"><div class="num">3</div><div class="lbl">Standalone Programs</div></div><div class="kpi"><div class="num">${attendance.lastSaved?new Date(attendance.lastSaved).toLocaleDateString():'-'}</div><div class="lbl">Attendance Last Saved</div></div></div><div class="grid cols-2">${['attendance','roster','tasks','other-programs'].map(id=>`<div class="card module-card" onclick="navigate('${id}')"><h3>${MODULES.find(m=>m.id===id).label}</h3><p>${id==='attendance'?'Daily entry, 90-day grid, exceptions, clear by person/code, and shared JSON storage.':id==='roster'?'Staff records, master schedule, uniforms, training, print/export, and shared JSON storage.':id==='tasks'?'Project/task tracker, weekly boss update email/export, status, priority, owner, and follow-up tracking.':'Launch standalone specialist tools: Badge Audit, AMAG Audit, and Access Audit. Keeps the suite stable while preserving each tool’s original workflow.'}</p></div>`).join('')}</div>`}

...
need(main,'private const string AppVersion = "4.2.1";','AppVersion must be 4.2.1 for this maintenance release.');
need(main,'EnsureLiveSchemaMetadata();','Startup schema metadata initialization is missing.');
for(const [id,revision] of [['attendance',2],['roster',1],['tasks',1],['shift-reports',1],['shift-intelligence',1],['suite-settings',2]]){
...
need(csproj,'<Version>4.2.1</Version>','Visible application package version must be 4.2.1.');
need(csproj,'<FileVersion>4.2.1.0</FileVersion>','Windows file metadata must be 4.2.1.0.');
need(csproj,'<AssemblyVersion>4.2.1.0</AssemblyVersion>','Windows assembly metadata must be 4.2.1.0.');
need(manifest,'version="4.2.1.0"','Windows manifest identity must be four-part 4.2.1.0.');
...
const seeds={
  'attendance-data.json':'attendance-2',
  ...
  'suite-settings.json':'suite-settings-2'
};
for(const [file,expected] of Object.entries(seeds)){
  ...
  if(obj.lastWrittenByAppVersion!=='4.2.1')throw new Error(`${file} lastWrittenByAppVersion is not 4.2.1`);
}
...
console.log('- Attendance and Suite Settings are registered at schema revision 2; other current modules remain at revision 1');
...
console.log('- Three-part app version 4.2.1 with four-part Windows metadata retained');

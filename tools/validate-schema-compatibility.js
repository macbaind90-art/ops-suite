...
need(main,'private const string AppVersion = "4.3.0";','AppVersion must be 4.3.0 for this maintenance release.');
need(main,'EnsureLiveSchemaMetadata();','Startup schema metadata initialization is missing.');
for(const [id,revision] of [['attendance',2],['roster',1],['tasks',1],['shift-reports',1],['shift-intelligence',1],['suite-settings',3]]){
...
need(csproj,'<Version>4.3.0</Version>','Visible application package version must be 4.3.0.');
need(csproj,'<FileVersion>4.3.0.0</FileVersion>','Windows file metadata must be 4.3.0.0.');
need(csproj,'<AssemblyVersion>4.3.0.0</AssemblyVersion>','Windows assembly metadata must be 4.3.0.0.');
need(manifest,'version="4.3.0.0"','Windows manifest identity must be four-part 4.3.0.0.');
...
for(const [file,expected] of Object.entries(seeds)){
  ...
  if(obj.lastWrittenByAppVersion!=='4.3.0')throw new Error(`${file} lastWrittenByAppVersion is not 4.3.0`);
}
...
console.log('- Attendance is at schema revision 2, Suite Settings is at revision 3, and other current modules remain at revision 1');
...
console.log('- Three-part app version 4.3.0 with four-part Windows metadata retained');

...
for(const token of [
  ...
  'attendance.pointSystem.pointValueHistory','Attendance point values updated','lastRecalculatedAt',
...
const attendance={
  ...
  attendance:{e1:{'2026-09-01':'T15+'}},notes:{},audit:[],pointAdjustments:[],correctiveActions:[],recordEdits:[],tardyReclassifications:{},autoOff:{},workdayBasis:{},
...
const sandbox={
  ...
  isArchivedEmployee:()=>false,rosterRdoToAttendanceRdos:()=>[],activeAttendanceEmployees:()=>attendance.employees,getCode:()=>'',audit(){},saveAttendance(){},esc:s=>String(s??''),fmt:s=>s,sortedEmployees:()=>attendance.employees,currentUserName:()=> 'Admin User',env:{user:'Admin User',machine:'QA'},
...
if(snap.activePoints!==1)throw new Error('Default T15+ point calculation changed unexpectedly: '+snap.activePoints);
...
if(snap.activePoints!==2.5)throw new Error('Existing attendance did not recalculate after point-value edit: '+snap.activePoints);
...
console.log('Attendance point-value editor validation: PASS');

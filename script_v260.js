...
'use strict';
const MODULES=[{id:'home',label:'Home'},{id:'attendance',label:'Attendance'},{id:'roster',label:'Roster'},{id:'tasks',label:'Task Tracker'},{id:'badge-audit',label:'Badge Audit'},{id:'amag-audit',label:'AMAG Audit'},{id:'access-audit',label:'Access Audit'}];
...
async function mockResponse(type,payload,extra){if(type==='suite:getSettings')return{settings:{...DEFAULT_SETTINGS},environment:{user:'Preview',machine:'Browser',version:'2.4.12'}};if(type==='suite:loadModuleData'&&extra.module==='attendance'){let r=await fetch('seed/attendance-data.json');return{module:'attendance',data:await r.text()}};if(type==='suite:loadModuleData'&&extra.module==='roster'){let r=await fetch('seed/roster-data.json');return{module:'roster',data:await r.text()}};if(type==='suite:loadModuleData'&&extra.module==='tasks'){let r=await fetch('seed/tasks-data.json');return{module:'tasks',data:await r.text()}};if(type==='suite:healthCheck')return{dataRoot:settings.dataRoot,checks:[{name:'Preview mode',ok:true,error:''},{name:'Desktop bridge unavailable in browser',ok:false,error:'Run EXE for real checks'}]};return{ok:true}}
...
function likelySamePerson(attEmp,re){
  ...
  const ap=attendanceNameParts(attEmp.name);
  ...
  if(rf&&af&&(rf===af||rf.startsWith(af)||af.startsWith(rf)||rf[0]===af[0]))return shiftCompatible(attEmp.shift, rosterShiftToAttendanceShift(re));
  return false;
}
...
function findAttendanceEmployeeForRoster(re){
  ...
  // Prefer a live roster link, but do not use archived/sync-duplicate rows as the match.
  const exact=(attendance.employees||[]).find(a=>!isArchivedAttendanceEmployee(a)&&String(a.rosterId||'')===rid);
  if(exact)return exact;
  // First-time linking: match by same person. Prefer the row with the most actual attendance history.
  const candidates=(attendance.employees||[]).filter(a=>!isArchivedAttendanceEmployee(a)&&!a.rosterId&&likelySamePerson(a,re)).sort((a,b)=>meaningfulAttendanceCount(b.id)-meaningfulAttendanceCount(a.id));
...
function renderModule(id){if(id==='home')return renderHome();if(id==='attendance')return renderAttendance();if(id==='roster')return renderRoster();if(id==='tasks')return renderTasks();if(id==='badge-audit'||id==='amag-audit'||id==='access-audit')return renderAuditTool(id);return `<div class="page-head"><div><div class="page-title">${MODULES.find(m=>m.id===id).label}</div><div class="page-sub">Module not available.</div></div></div>`}
...
function renderBadgeAuditNative(){
  const f=badgeAuditState.files||{};
  const fileLabel=k=>f[k]?esc(f[k].name):'No file loaded';
  const findings=badgeAuditState.findings||[];
  return `<div class="native-audit">
    <div class="page-head">
      <div><div class="page-title">Badge Audit</div><div class="page-sub">Native suite audit module · upload punch details and badge reader logs, run review, export findings</div></div>
      <div class="btn-group">
        <button onclick="runNativeBadgeAudit()">Run Audit</button>
        <button onclick="exportNativeBadgeAuditCSV()">Export CSV</button>
        <button onclick="printNativeBadgeAudit()">Print Report</button>
        <button onclick="clearNativeBadgeAudit()">Clear</button>
      </div>
    </div>
    <div class="drop-grid">
      ${renderAuditDrop('punch','Punch Details','Required. XLSX/CSV export from time system. CSV is parsed natively now; XLSX native parsing will be added in the next audit engine pass.','.xlsx,.xls,.csv')}
      ${renderAuditDrop('ts','TS Reader Log','Badge reader text log.','.txt,.csv')}
      ${renderAuditDrop('gate','Gate Reader Log','Badge reader text log.','.txt,.csv')}
      ${renderAuditDrop('dispatch','Dispatch Reader Log','Optional badge reader text log.','.txt,.csv')}
      ${renderAuditDrop('recep','Reception Reader Log','Optional badge reader text log.','.txt,.csv')}
    </div>
    <div class="card">
      <div class="card-title">Audit Settings</div>
      <div class="form-grid">
        <label>Badge-before-clock threshold minutes<input id="badgeAuditBadgeThreshold" type="number" min="0" value="1"></label>
        <label>Shift early threshold minutes<input id="badgeAuditShiftThreshold" type="number" min="0" value="10"></label>
        <label>Supervisor filter<select id="badgeAuditSupervisor"><option value="ALL">All Supervisors</option></select></label>
        <label>Audit date<input id="badgeAuditDate" type="date"></label>
      </div>
      <div class="audit-status">${badgeAuditState.ranAt?`Last run: ${esc(new Date(badgeAuditState.ranAt).toLocaleString())}`:'Load files, then run the audit.'}</div>
    </div>
    <div class="grid cols-4">
      <div class="kpi"><div class="num">${findings.length}</div><div class="lbl">Total Findings</div></div>
      <div class="kpi"><div class="num">${findings.filter(x=>x.type==='Clock-in Before Badge').length}</div><div class="lbl">Clock-in Before Badge</div></div>
      <div class="kpi"><div class="num">${findings.filter(x=>x.type==='No Badge Match').length}</div><div class="lbl">No Badge Match</div></div>
      <div class="kpi"><div class="num">${Object.values(f).filter(Boolean).length}</div><div class="lbl">Files Loaded</div></div>
    </div>
    <div class="card">
      <div class="card-title">Loaded Files</div>
      <div>${['punch','ts','gate','dispatch','recep'].map(k=>`<span class="result-pill"><strong>${esc(k.toUpperCase())}</strong> ${fileLabel(k)}</span>`).join('')}</div>
    </div>
    <div class="card">
      <div class="card-title">Findings</div>
      ${findings.length?`<div class="table-wrap"><table><thead><tr><th>Type</th><th>Employee</th><th>Date</th><th>Clock</th><th>Badge</th><th>Source</th><th>Notes</th></tr></thead><tbody>${findings.map(x=>`<tr><td>${esc(x.type)}</td><td>${esc(x.employee)}</td><td>${esc(x.date)}</td><td>${esc(x.clock)}</td><td>${esc(x.badge)}</td><td>${esc(x.source)}</td><td>${esc(x.notes)}</td></tr>`).join('')}</tbody></table></div>`:`<p class="muted">No findings yet. Run the audit after loading files.</p>`}
    </div>
  </div>`;
}
...
function renderHome(){const flags=computeFlags();return `<div class="page-head"><div><div class="page-title">Security Operations Dashboard</div><div class="page-sub">${env.user||'User'} @ ${env.machine||'Machine'} · v2.6.0</div></div></div><div class="grid cols-3"><div class="kpi"><div class="num">${activeAttendanceEmployees().length}</div><div class="lbl">Active Attendance Employees</div></div><div class="kpi"><div class="num">${roster.employees.length}</div><div class="lbl">Roster Employees</div></div><div class="kpi"><div class="num">${flags.filter(f=>!f.addressed).length}</div><div class="lbl">Open Attendance Exceptions</div></div><div class="kpi"><div class="num">${(tasks.tasks||[]).filter(t=>t.status!=='Complete'&&t.status!=='On Hold').length}</div><div class="lbl">Active Tasks</div></div><div class="kpi"><div class="num">${attendance.lastSaved?new Date(attendance.lastSaved).toLocaleDateString():'-'}</div><div class="lbl">Attendance Last Saved</div></div></div><div class="grid cols-2">${['attendance','roster','tasks','badge-audit','amag-audit','access-audit'].map(id=>`<div class="card module-card" onclick="navigate('${id}')"><h3>${MODULES.find(m=>m.id===id).label}</h3><p>${id==='attendance'?'Daily entry, 90-day grid, exceptions, clear by person/code, and shared JSON storage.':id==='roster'?'Staff records, master schedule, uniforms, training, print/export, and shared JSON storage.':id==='tasks'?'Project/task tracker, weekly boss update email/export, status, priority, owner, and follow-up tracking.':id==='badge-audit'?'Native badge audit module with suite-owned workflow and exports.':id==='amag-audit'?'Native AMAG audit capture screen. Full comparison engine next.':'Native access audit capture screen. Full comparison engine next.'}</p></div>`).join('')}</div>`}
...
function rosterShifts(){return [...new Set(rosterActiveEmployees().map(e=>e.shift||'').filter(Boolean))].sort()}
